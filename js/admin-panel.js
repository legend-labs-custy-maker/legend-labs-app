...
import { refreshGrid, refreshCategoryBars, updateMaintenanceScreen } from './shop.js';
...
function refreshPublicBanners() {
  UI.renderBannerRow(state.banners.filter(b => b.is_active), (link) => Cart.goToCheckout(link, tg));
}
...
document.getElementById('bnFile').addEventListener('change', async (e) => {
  ...
  try {
    ...
    const res = await Admin.createBanner({ image_url, link_url: linkUrl || null, sort_order: state.banners.length, is_active: true });
