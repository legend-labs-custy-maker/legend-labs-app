...
import { state, tg, haptic, open, close, isSoundEnabled, setSoundEnabled, isAutoplayEnabled, setAutoplayEnabled } from './state.js?v20260724c';
import * as Products from './products.js?v20260724c';
import * as Cart from './cart.js?v20260724c';
import * as UI from './ui.js?v20260724c';
import { t, getLang, setLang, applyTranslations } from './i18n.js?v20260724c';
import { registerUser } from './api.js?v20260724c';
import * as Favorites from './favorites.js?v20260724c';
...
export async function loadEverything() {
  const [settings, categories, subcategories, products, promotions, contactLinks, reviews, notifications, banners] = await Promise.all([
    ...
    safeLoad(Products.loadNotifications(), [], 'notifications'),
    safeLoad(Products.loadBanners(), [], 'bannières'),
  ]);
...
export function refreshGrid() {
  ...
  if (homeEl) homeEl.style.display = hasActiveSearch ? 'none' : 'block';
  ...
  UI.renderGrid(getFilteredProducts(), {
    ...
    onFavorite: handleFavorite,
    isFavorite: Favorites.isFavorite,
    videoAutoplay: isAutoplayEnabled(),
  });
}
...
function renderCurrentProduct() {
  ...
  UI.renderProductDetail(product, {
    ...
    onSelectVariant: (variantId) => { state.currentVariantId = variantId; pdQty = 1; renderCurrentProduct(); },
    onAddToCart: (variantId, qty) => { addToCart(variantId, qty); close('product'); },
    ...
    qty: pdQty,
    onQtyChange: (newQty) => { pdQty = newQty; renderCurrentProduct(); },
  });
  UI.renderSimilarProducts(state.products, product, (productId) => {
    state.currentProductId = productId;
...
function initSettingsTab() {
  ...
  if (autoplayToggle) {
    ...
    autoplayToggle.addEventListener('change', () => { setAutoplayEnabled(autoplayToggle.checked); refreshGrid(); haptic('light'); });
  }
}

// ---------- Centre de notifications ----------
const NOTIF_SEEN_KEY = 'legendlabs_notif_seen';
function getLastSeenNotifDate() {
  return localStorage.getItem(NOTIF_SEEN_KEY) || '1970-01-01T00:00:00Z';
}
function computeUnreadNotifCount() {
...
document.getElementById('notifBell').addEventListener('click', () => {
  ...
  if (state.notifications.length) {
    try { localStorage.setItem(NOTIF_SEEN_KEY, state.notifications[0].created_at); } catch { /* pas grave */ }
  }
  UI.renderNotifBadge(0);
});
...
async function registerCurrentUser() {
  if (!tg || !tg.initData) { renderProfileView(); return; }
  try {
...
function renderProfileView() {
  UI.renderProfileHeader(state.currentUser);
  const favProducts = state.products.filter(p => Favorites.isFavorite(p.id));
  UI.renderProfileFavorites(favProducts, (productId) => {
    document.querySelectorAll('.tab').forEach(t => t.classList.remove('active'));
    document.querySelector('.tab[data-tab="home"]').classList.add('active');
    document.querySelectorAll('.view').forEach(v => v.classList.remove('active'));
    document.getElementById('view-home').classList.add('active');
