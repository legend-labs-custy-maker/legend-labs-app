...
const sheets = {
  ...
  adminProductEdit: [document.getElementById('sheetAdminProductEdit'), document.getElementById('backdropAdminProductEdit')],
};
...
function openAdminPanel() {
  ...
  UI.renderAdminProducts(state.products, { onDelete: handleDeleteProduct, onManage: openProductEditor });
...
document.getElementById('addContactLinkBtn').addEventListener('click', async () => {
...
// ============================================================
// ÉDITEUR PRODUIT (médias, variantes, lots)
// ============================================================
let editingProductId = null;

function getEditingProduct() {
  return state.products.find(p => p.id === editingProductId) || null;
}
...
function openProductEditor(productId) {
  ...
  open('adminProductEdit');
}

function renderEditorSections() {
  const product = getEditingProduct();
  if (!product) return;
  UI.renderProductEditorTitle(product);
  UI.renderMediaList(product.media, handleDeleteMedia);
  UI.renderVariantEditor(product.variants, {
...
document.getElementById('peMediaFile').addEventListener('change', async (e) => {
  const product = getEditingProduct();
  if (!product) return;
  const files = Array.from(e.target.files || []);
  for (const file of files) {
    if (file.size > 6 * 1024 * 1024) { alert(`"${file.name}" dépasse 6 Mo, ignoré.`); continue; }
    try {
      const file_base64 = await fileToBase64(file);
      const res = await Admin.uploadMedia({ product_id: product.id, file_base64, file_name: file.name, content_type: file.type });
      product.media.push(res.data);
    } catch { haptic('warning'); alert(`Échec de l'upload de "${file.name}".`); }
  }
  e.target.value = '';
  renderEditorSections();
  refreshGrid();
...
async function handleDeleteMedia(id) {
  const product = getEditingProduct();
  try {
    await Admin.deleteMedia(id);
    product.media = product.media.filter(m => m.id !== id);
    renderEditorSections();
    refreshGrid();
    haptic('medium');
...
document.getElementById('addVariantBtn').addEventListener('click', async () => {
  ...
  const unit = document.getElementById('pvUnit').value;
  const price = parseFloat(document.getElementById('pvPrice').value);
  const stock = parseInt(document.getElementById('pvStock').value, 10) || 0;
  if (!name || isNaN(price) || price < 0) { haptic('warning'); return; }
  try {
    const res = await Admin.createVariant({ product_id: product.id, name, unit, price, stock, sort_order: product.variants.length });
    res.data.lots = [];
    product.variants.push(res.data);
    document.getElementById('pvName').value = '';
    document.getElementById('pvPrice').value = '';
    document.getElementById('pvStock').value = '';
    renderEditorSections();
    refreshGrid();
    haptic('success');
...
async function handleAddLot(variantId, quantity, price) {
  ...
  try {
    ...
    variant.lots = (variant.lots || []).concat(res.data);
    renderEditorSections();
    refreshGrid();
    haptic('success');
...
async function handleDeleteLot(lotId) {
  ...
  try {
    ...
    product.variants.forEach(v => { v.lots = (v.lots || []).filter(l => l.id !== lotId); });
    renderEditorSections();
    refreshGrid();
    haptic('medium');
